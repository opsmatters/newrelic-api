...
public class PluginDeserializer implements JsonDeserializer<Plugin>
{
     ...
     */
    @Override
    public Plugin deserialize(JsonElement element, Type type, JsonDeserializationContext context)
        throws JsonParseException
    {
        JsonObject obj = element.getAsJsonObject();
        JsonElement plugin = obj.get("plugin");
        if(plugin != null && plugin.isJsonObject())
            return gson.fromJson(plugin, Plugin.class);
        return gson.fromJson(element, Plugin.class);
    }
}
